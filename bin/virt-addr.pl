...
# Get the IP address of a virtual machine based on 
# it's name
...
if ( ($#ARGV) >= 1 and ($ARGV[1] ne "") ) {
    $ENV{LIBVIRT_DEFAULT_URI} = "qemu+ssh://$ARGV[1]/system";
}
else {
...
my $connect = "";
if ($ENV{LIBVIRT_DEFAULT_URI} =~ /ssh/) {
    $connect = "ssh $ARGV[1] ";
}

my $out = `$connect /usr/sbin/arp -an`;
my @arp_lines = split (/\n/, $out);
foreach (@arp_lines) {
    if (/\((.*?)\) at (.*?) /) {
        my $this_addr = lc $2;
